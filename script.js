...
async function checkSupabaseConnection() {
  ...
  if (!supabaseConnected) {
    ...
    activePlayersStatus.textContent = "NOT CONNECTED";
  }
}
...
async function sendHeartbeat() {
  ...
  try {
    await requestJson("/players/heartbeat", {
      method: "POST",
...
async function removeActivePlayer() {
  ...
  try {
    await requestJson("/players/remove", {
      method: "POST",
...
function renderLeaderboard(entries) {
  leaderboardRows.innerHTML = entries.length ? entries.map((entry, index) => `<div class="leaderboard-row" role="row"><span class="rank">${rankLabel(index + 1)}</span><strong>${escapeHtml(entry.nickname)}</strong><span>${escapeHtml(modes[entry.mode]?.name || entry.mode)}</span><b>${entry.score}</b><span class="combo-value">${entry.highest_combo}</span></div>`).join("") : '<p class="empty-board">No verified scores yet.</p>';
}
...
function renderActivePlayers(players) {
  activePlayersRows.innerHTML = players.length ? players.map((player) => `<div class="active-player"><span class="online-dot"></span><strong>${escapeHtml(player.nickname)}</strong><span>${escapeHtml(modes[player.mode]?.name || player.mode)}</span><b>${player.score}</b></div>`).join("") : '<p class="empty-live">No other players are online.</p>';
  activePlayersStatus.textContent = `${players.length} PLAYING`;
}

async function loadActivePlayers() {
  try {
    const data = await requestJson("/players");
    renderActivePlayers(data.players);
  } catch (error) {
    activePlayersRows.innerHTML = "";
    activePlayersStatus.textContent = "SUPABASE NOT CONNECTED";
  }
}
...
loadLeaderboard();
loadActivePlayers();
livePlayersTimer = setInterval(loadActivePlayers, 5000);
